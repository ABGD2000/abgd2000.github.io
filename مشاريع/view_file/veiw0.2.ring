...
new qApp {
    ...
    displayFileContent("/home/hassan2harby/hassan document/whatapp backup 25 11/Media/WhatsApp Audio/AUD-20251031-WA0012.mp3")
...
func displaySoundFile(fileName)
    try
        ...
        groupInfo = new qVBoxLayout() {
                addWidget(new qLabel(win1){setText("File: " + fileName)})
                addWidget(new qLabel(win1){setText("Type: " + getFileExtension(fileName))})
                addWidget(new qLabel(win1){setText("Sound files can be played using external applications")})
            }
        
        contentLayout.addLayout(groupInfo)
        
        // Play button (conceptual - would need sound library for actual playback)
        btnPlay = new qPushButton(win1){
            setText("🎵 Play Sound (External)") 
            setStyleSheet("
                QPushButton {
        ...
        btnPlay.setclickEvent("playSound('"+fileName+"')")
...
func playSound (fileName)
     ...
     new qmediaplayer()
         {
           setmedia(new qurl("file:///"+fileName))
           setvolume(50) play()
         }
